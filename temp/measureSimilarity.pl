...
for($i=0;$i<scalar(@targetwords);$i++){
	...
	$contextvectors{%tw} = \@curvector;
		...
		if($paircounts{"$tw\t$fw"} > 0){
			$pxy = $paircounts{"$tw\t$fw"}/$totalNumberOfPairs;
			$px = $individualcounts{"tw"}/$totalNumberOfWords;
			$py = $individualcounts{"fw"}/$totalNumberOfWords;
			if($px == 0)
			{
			  $px = 1/$totalNumberOfWords;
			}
			if($py == 0)
			{
			  $py = 1/$totalNumberOfWords;
			}
			$pmi = log($pxy/($px*$py))/log(2);  #<------- WRITE FORMULA HERE
		}else{
			$pmi = 0;
		}
